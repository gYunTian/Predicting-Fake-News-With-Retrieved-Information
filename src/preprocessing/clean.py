# An extremely inefficient cleaning toolkit
# this file will apply the clean function on each row in a .txt file
# to adapt this for use, in the if __name__ == __main__ part, point the with open command to where the text files are stored
...
def to_normalize(data):
    ...
    # Global
    data = list(map(lambda x: _make_cleaning(x,normalized_chars), data))
...
def remove_control_char(data):
    ...
    data = list(map(lambda x: ' '.join([_make_cleaning(i,chars_dict) for i in x.split(' ')])
    , data))
...
def remove_starting_underscore(data):
    if verbose: print('#' * 10, 'Step - Remove starting underscore:')
    local_vocab = {}
    temp_vocab = _check_vocab(data, local_vocab, response='unknown_list')
    temp_vocab = [k for k in temp_vocab if (_check_replace(k)) and ('_' in k)]
    temp_dict = {}
...
def seperate_start_word_punctuations(data):
    if verbose: print('#' * 10, 'Step - Start word punctuations:')
...
def remove_s(data):
    if verbose: print('#' * 10, 'Step - Remove "s:')
    local_vocab = {}
    temp_vocab = _check_vocab(data, local_vocab, response='unknown_list')
    temp_vocab = [k for k in temp_vocab if _check_replace(k)]
...
def isolate_numbers(data):
    if verbose: print('#' * 10, 'Step - Isolate numbers:')
    local_vocab = {}
    temp_vocab = _check_vocab(data, local_vocab, response='unknown_list')
    temp_vocab = [k for k in temp_vocab if _check_replace(k)]
    temp_dict = {}
...
def worker_post_clean(text):
  global cleaned
  prev = len(text)
  text = clean_v1(text)
  new = len(text)
  if (new < prev): print('Removed:',prev - new)
...
if __name__=='__main__':
    ...
    print("------ Starting Post Cleaning ------")
    for i in range(16,31):
      print("Cleaning doc", i)
      try:
        with open(os.path.join('../output_fixed/', 'conrefed_'+str(i)+'.txt'), 'r') as f:
          text = f.read()
          data = text.split('</end>')
          for line in data:
            tasks.append(line)
        ...
        pool.map(worker_post_clean, tasks)
        pool.close()
        pool.join()
        tasks = list()

        with open(os.path.join('../output_post/', 'post_'+str(i)+'.txt'), 'a', encoding='utf-8') as out:
          for line in cleaned:
            out.write(line + "\n")
            out.write('</end>\n')
        cleaned = list()
